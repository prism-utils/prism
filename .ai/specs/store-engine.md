...
- [x] **Gate 2 — Edge cases:** empty window; flush with empty hot_prev; first flush when `hot_current` absent; missing tables (`does not exist`/`Catalog Error`) treated as empty via `strings.Contains`; legacy import when `metrics-raw/` absent; concurrent ingest+flush under `-race`; LRU eviction closing an in-use handle is avoided.
...
- [x] Full docs/REVIEW.md checklist passes; TESTING.md layering respected (unit + golden where a parquet fixture is asserted).
...
**APPROVE** (2026-07-22, re-review). Fixes verified: `abcd875` wires CI lint to `make lint` (install golangci-lint v2.12.0 + `CGO_ENABLED=1`); `365b358` adds `edge_test.go` with real assertions for empty hot_prev flush, absent hot_current, absent metrics-raw/, and concurrent ingest+flush (40-row accounting + post-race ingest). `CGO_ENABLED=0 golangci-lint` still fails on go-duckdb typecheck; `make lint`/`make test` green; builds/vet pass; whole diff remains #24 engine scope (12 files).
