...
| `COLD_AFTER` | duration | `12h` | Age ruler for promote: a compacted L0+ file may leave `DATA_DIR` when its catalog/footer `max_ts` is at least this old. Go duration (`12h`, `6h`). Aged L0 is force-packed (same type) when possible, then leftover single/skip/sealed L0 is still eligible. Ignored when `COLD_DATA_DIR` is empty. Zero/unparsable values keep the default. |
| `COLD_DATA_DIR` | string | _(empty — off)_ | Second store root for compacted metrics and logs as **parquet**. Hot snapshots, landing, rollups, materializations, and manifests stay on `DATA_DIR`. Empty/unset is today's single-root layout. The writer (`RUN_JOBS=true`) places eligible files crash-safely (`*.promote.tmp`, fsync, same-FS rename): `.parquet` sources are byte-copied (SHA-256 + PAR1); L1+ `.duckdb` sources are converted to `.parquet` on the cold filesystem (PAR1); leftover aged L0 parquet may promote; L0 `.duckdb` never leaves hot. The hot source is never unlinked until dest verifies. Read replicas (`RUN_JOBS=false`) only **read** both roots. |
| `COMPACT_AGE_CATCHUP` | bool | `true` | When Lucene finds no metrics merge, pack fully-aged unsealed files (`max_ts ≤ now−15m`) without time-adjacency, up to 32 sources or 256 MiB, dest `L{n+1}` on `DATA_DIR`. Files that do not fit remaining `maxBytes` are skipped. `false` disables the built-in packer; named `COMPACT_FILE` policies still run. |
| `COMPACT_AGE_CATCHUP_MAX_BYTES` | bytes | `256Mi` | Catch-up pack cap. Lower this when DuckDB COPY of a 256Mi pack OOMs; files larger than the cap are skipped so later files can still pack. |
| `COMPACT_FILE` | path | _(empty)_ | YAML named compact policies (`compact.policies[]`: `name`, `tier`, `olderThan`, `newerThan`, `bucket` none\|hour\|day UTC, `maxSources` ≥ 2, `maxBytes` > 0, `every`). Unset/empty = no named policies. Invalid or duplicate names fail start. See [`STORE.md`](STORE.md#tiered-compaction-internalstoremerge). |
...
| `HOT_SEGMENT_FORMAT` | string | `parquet` | On-disk format for the metrics hot snapshot (`hot/current.parquet` or `hot/current.duckdb`). Values: `parquet` \| `duckdb`. Invalid values fail startup. Live `engine.duckdb` is unchanged; this is the sandbox/replica export only. Binary default stays `parquet` for back-compat. Recommended writer env when `COLD_DATA_DIR` is set: `duckdb` (cold dest is still parquet via promote convert). |
| `MERGE_SEGMENT_FORMAT` | string | `parquet` | On-disk format for metrics flush→L0 and parquet-source metrics/logs tier merges (`.parquet` or `.duckdb`). DuckDB-source packs stay `.duckdb` regardless of this value (same-type merge; dest extension follows payload magic). Values: `parquet` \| `duckdb`. Invalid values fail startup. Binary default stays `parquet`. Recommended writer env when `COLD_DATA_DIR` is set: `duckdb` (L1+ promote converts to parquet on cold). |
...
| `LOGS_DELETE_GRACE_SECONDS` | int (seconds) | `120` | How long a merged-away segment keeps its bytes at its original path before the merge tick unlinks it. A client that globs the tier tree (the Grafana DuckDB datasource) resolves paths at bind time and opens them during execution; `read_parquet` has no `ignore_missing`, so a source deleted in between fails the whole query. Applies to log **and** metrics merge sources. `0` deletes immediately (the behavior before this knob existed) — reach for it only to reclaim space urgently. Cost: up to one grace window of compacted sources on disk, and a globbing client can see those rows twice until the purge; prism's own `/sql`, Loki, and PromQL never do. |
...
| `LOGS_REFRESH_MAX_ACTIONS` | int | `8` | Landing→L0 refreshes applied per artifact per merge tick. Each action rewrites a pack of buffered windows, so this bounds refresh CPU while still letting a backlog shrink faster than agents fill it. When this is ≥ 2 and the live landing set exceeds one pack, the first action is newest-first (remaining actions stay oldest-first) so last-hour queries recover before the drain finishes. |
...
| `MEMORY_OBSERVE` | bool | `false` | Extra memory-debug series on `/metrics` (cgroup v2, DuckDB open counts by role, per-job RSS/heap snapshots) plus one slog line per lifecycle job. Independent of `METRICS_ENABLED`; inert when metrics are off (no scrape). Default scrape is unchanged when this is false. |
...
| `POST` | `/admin/tenants/{tenant}/compact` | `200` dry-run JSON / `202` `{queued:true}` / `204` when `RUN_JOBS=false` | admin plane; window compact (policy name or inline selector). `400` missing window; `404` unknown tenant. Bearer/`ensure` RBAC same as ensure. |
