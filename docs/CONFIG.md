...
| `COLD_DATA_DIR` | string | _(empty — off)_ | Second store root for compacted L0+ metrics and logs. Hot snapshots, landing, rollups, materializations, and manifests stay on `DATA_DIR`. Empty/unset is today's single-root layout. The writer (`RUN_JOBS=true`) copies eligible files crash-safely (`*.promote.tmp`, fsync, SHA-256, parquet/duckdb magic, same-FS rename) and never unlinks the hot source until dest verifies. Read replicas (`RUN_JOBS=false`) only **read** both roots. |
