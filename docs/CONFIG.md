...
| `COMPACT_AGE_CATCHUP` | bool | `true` | When Lucene finds no metrics merge, pack fully-aged unsealed files (`max_ts ≤ now−15m`) without time-adjacency, up to 32 sources or 256 MiB, dest `L{n+1}` on `DATA_DIR`. `false` disables the built-in packer; named `COMPACT_FILE` policies still run. |
