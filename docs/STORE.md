...
- **Metrics rewrite:** parquet sources with identical footers are concatenated (one row group at a time, `MinTs` order). DuckDB `COPY (… ORDER BY ts)` runs only when concat cannot (`.duckdb` format, corrupt footer, or a test-forced failure). Schema mismatches split the pack: the first homogeneous group of two or more files is rewritten; a leftover singleton stays live. After five failed rewrites a `<segment>.merge-skip` sidecar keeps the file out of `ScanTier` / merge planning while queries still read it.
- **Logs rewrite:** parquet sources are k-way merged by `__prism_ts_ns` (stamped from the source window when the column is missing). Missing columns are null-filled. DuckDB `UNION ALL BY NAME` + `ORDER BY __prism_ts_ns` is the fallback, with the same five-attempt skip sidecar.
